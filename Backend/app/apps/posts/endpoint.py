...
@post_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=List[schema.PostList]
)
async def post_list(
    ...
    if "admin" in str(current_user.scopes) and current_user.is_superuser:
        posts = post_dal.get_multi(
            session=session, skip=skip, limit=limit + 10, model=User
        )
    else:
        posts = post_dal.get_multi_with_author(
...
@post_router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schema.PostDetail
)
...
@post_router.get(
    "/{post_id}", status_code=status.HTTP_200_OK, response_model=schema.PostDetail
)
async def post_detail(
    *,
    ...
    return post_dal.get_object_with_relations_or_404(
        session=session, instance_id=post_id
    )


@post_router.patch(
    "/{post_id}", status_code=status.HTTP_201_CREATED, response_model=schema.PostDetail
)
async def post_update(
    *,
    ...
    db_obj: model.Post = post_dal.get_object_or_404(
        session=session, instance_id=post_id
    )
    ...
    post = post_dal.update(session=session, obj_in=obj_in, session_model=db_obj)
    return post


@post_router.delete(
    "/{post_id}", status_code=status.HTTP_200_OK, response_model=schemas.Msg
)
